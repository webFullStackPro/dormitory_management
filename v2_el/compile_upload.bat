...
set COMPRESSED_FILE_NAME=dormitory_management_v2_el.zip
...
set REMOTE_SCRIPT_PATH=/home/ecs-user/ws/dormitory_management/dormitory_management_v2_el.sh
...
npm run build && 7z.exe a -tzip %COMPRESSED_FILE_NAME% ./dist/* && %PSCP_PATH% -scp -i %PRIVATE_KEY_PATH% %COMPRESSED_FILE_NAME% %REMOTE_USER%@%REMOTE_SERVER%:%REMOTE_DIR% && plink.exe -ssh -i "%PRIVATE_KEY_PATH%" -batch ecs-user@8.136.123.102 "sh %REMOTE_SCRIPT_PATH%"
